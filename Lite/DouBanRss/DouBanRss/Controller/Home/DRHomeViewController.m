...
//    self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"分享" style:UIBarButtonItemStyleBordered target:self action:@selector(myButtonHandlerAction:)] autorelease];
    ...
    tableView1 = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, screenWidth, screenHeight-80)];
    tableView1.delegate = self;
    tableView1.backgroundColor = RGBA(255, 241, 224, 1);
    ...
    CGRect frame2 = adView.frame;
    frame2.origin.x = 0;
    
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ) {
       adView = [[YouMiView alloc] initWithContentSizeIdentifier:YouMiBannerContentSizeIdentifier200x200 delegate:nil];
    }
    else {
    ...
    adView.appID	= youmiAppId; // 该地方请填写您直接的APP ID 
    adView.appSecret = youmiSecretid; // 该地方请填写您直接的APP Secret
    adView.appVersion = p_version;
